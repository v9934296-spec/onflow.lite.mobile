...
export default function PteFlowScreen() {
  ...
  const localRatings = useMemo(
    () =>
      log
        .filter((entry) => entry.analysis.source === "user" && entry.analysis.rating != null)
        .map((entry) => entry.analysis.rating as number)
        .slice(-12),
    [log],
  );
  ...
  const latestBreakdown = useMemo(
    () =>
      [...log]
        .reverse()
        .find((entry) => entry.analysis.source === "user" && (entry.analysis.breakdown?.length ?? 0) > 0)
    ...
    <View style={[s.screen, { paddingTop: insets.top + 12, paddingBottom: insets.bottom }]}> 
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        <View style={s.header}>
          ...
          <Text style={s.sub}>Self-reported consistency and video evidence stay separate. PTE only scores what the footage can actually support.</Text>
        </View>
        ...
        <View style={s.metricRow}>
          ...
          <View style={s.metric}>
            <Text style={s.metricValue}>{stats.landed}</Text>
            <Text style={s.metricLabel}>LANDS</Text>
          </View>
          <View style={[s.metric, s.metricRed]}>
            <Text style={s.metricValue}>{pct(stats.landed, stats.total)}</Text>
            <Text style={s.metricLabel}>LAND RATE</Text>
          </View>
        </View>
        ...
        <Card accent={C.volt}>
          ...
          <WeekRow days={week} />
          <Text style={s.note}>Green = a completed session with a reported land. Pink = attempts without a land. This is session consistency, not a PTE score.</Text>
        </Card>

        <Card>
          <Eyebrow color={C.volt}>VIDEO PTE TREND</Eyebrow>
          {timelineLoading && !ratings.length ? (
            <View style={s.loading}>
              <ActivityIndicator color={C.volt} />
              <Text style={s.note}>Loading progression evidence…</Text>
            </View>
          ) : ratings.length ? (
            <>
              <RatingLine ratings={ratings} />
              <Text style={s.note}>Plotted from analyzed clip/session PTE results only. No usable evidence means no invented score.</Text>
            </>
          ) : (
            <View style={s.empty}>
              <Text style={s.emptyTitle}>No video-rated attempts yet.</Text>
              <Text style={s.note}>Film attempts from Flow. Once a clip supports a rating, the trend appears here.</Text>
            </View>
          )}
        ...
        {latestBreakdown?.length ? (
          <Card accent={C.red}>
            <Eyebrow color={C.red}>LATEST MECHANICS BREAKDOWN</Eyebrow>
            <BreakdownBars items={latestBreakdown} />
            <Text style={s.note}>These bars come from the latest analyzed user clip that returned a mechanics breakdown.</Text>
          </Card>
        ) : null}
            ...
            {timeline.slice(0, 5).map((item) => (
              <View key={item.session_id} style={s.timelineRow}>
                <View style={{ flex: 1, gap: 2 }}>
                  <Text style={s.trickName}>{item.focus_trick ?? "Session"}</Text>
                  <Text style={s.note}>{shortDate(item.ended_at)}{item.spot ? ` · ${item.spot}` : ""} · {item.clips_count} clips</Text>
                </View>
        ...
        <View style={s.section}>
          ...
          {stats.tricks.length ? (
            stats.tricks.map((row, index) => (
              <View key={row.trick} style={s.trickRow}>
                <Text style={s.rank}>{String(index + 1).padStart(2, "0")}</Text>
                <View style={{ flex: 1 }}>
                  <Text style={s.trickName}>{row.trick}</Text>
                  <Text style={s.note}>{row.entries} attempts · {row.lands} landed</Text>
                </View>
                <Text style={s.trickRate}>{pct(row.lands, row.entries)}</Text>
              </View>
            ))
          ) : (
            <Text style={s.note}>Complete a Flow session and your trick progression will build here.</Text>
          )}
        </View>

        {momentum ? (
          <Card accent={C.red}>
            <Eyebrow color={C.red}>CURRENT BATTLE CANDIDATE</Eyebrow>
            <Text style={s.focus}>{momentum.trick}</Text>
            <Text style={s.sub}>{momentum.entries} attempts · {momentum.rate} land rate. Your most-worked trick is ready for a focused Battle session.</Text>
          </Card>
        ) : null}
      </ScrollView>
      <AppNav />
...
const s = StyleSheet.create({
  ...
  content: { paddingHorizontal: 20, paddingTop: 16, paddingBottom: 28, gap: 18 },
  header: { gap: 8, paddingBottom: 4 },
  title: { fontFamily: F.heading, fontSize: 36, lineHeight: 40, color: C.offwhite, maxWidth: 520 },
  ...
  metricGreen: { borderTopColor: C.volt },
  metricRed: { borderTopColor: C.red },
  metricValue: { fontFamily: F.heading, fontSize: 24, color: C.offwhite },
  metricLabel: { fontFamily: F.mono, fontSize: 9, letterSpacing: 0.9, color: C.dim, marginTop: 3 },
  section: { gap: 10 },
