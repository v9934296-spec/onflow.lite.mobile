    ...
    <View style={[s.screen, { paddingTop: insets.top + 12, paddingBottom: insets.bottom }]}> 
      <View style={s.content}>
        <View style={{ gap: 6 }}>
          ...
          <Text style={s.sub}>Progress worth seeing — sessions, clips, battles and breakthroughs.</Text>
        </View>

        {loading ? (
          <View style={s.centered}><ActivityIndicator color={C.volt} /><Text style={s.hint}>Loading feed…</Text></View>
        ) : error ? (
          <View style={s.centered}><Text style={s.error}>{error}</Text><Btn label="Retry" variant="ghost" onPress={() => void retry()} /></View>
        ) : showEmpty ? (
          <View style={s.centered}>
            <Card accent={C.volt}>
              <Eyebrow color={C.volt}>YOUR CIRCLE</Eyebrow>
              <Text style={s.emptyTitle}>Nothing fake to fill the feed.</Text>
              <Text style={s.hint}>Session recaps and real progression events will appear here as your circle skates.</Text>
            </Card>
          </View>
        ) : (
          <FlatList
            ...
            ListEmptyComponent={<Text style={s.hint}>No feed events yet. Check back after your next session.</Text>}
          />
        )}
      </View>
      <AppNav />
...
const s = StyleSheet.create({
  ...
  content: { flex: 1, paddingHorizontal: 24, paddingTop: 16, gap: 14 },
