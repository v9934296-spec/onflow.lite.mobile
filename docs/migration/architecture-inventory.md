...
> **Superseded snapshot.** This inventory reflects an earlier onflow-lite layout
> (`flow.ts`, local session history, home-centric session UX, no RevenueCat client).
> Current product architecture in this repo (`onflowdemo`) adds Flow / PTE tabs,
> server-backed progression history, pending-analysis and session-completion
> recovery, fail-closed user-scoped storage, and RevenueCat billing. Keep this
> file for migration history; do not treat it as the live system map.

Recorded at migration baseline. Describes the **new app as it existed at that
checkpoint**, not the source `ai-pop` codebase and not the current app.
